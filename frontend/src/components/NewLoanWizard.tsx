      ...
      if (loan?.id) {
        try {
          await apiService.download(
            `/api/loans/${loan.id}/contract`,
            `contrato-prestamo-${loan.id.substring(0, 8)}.pdf`
          );
        } catch (downloadError) {
          console.error('Error downloading contract after loan creation:', downloadError);
          alert('Préstamo originado exitosamente, pero no se pudo descargar el contrato.');
          window.location.href = `/prestamos/detalle?id=${loan.id}`;
          return;
        }

        alert('Préstamo originado y contrato descargado exitosamente.');
        window.location.href = `/prestamos/detalle?id=${loan.id}`;
      }
    } catch (err: any) {
        ...
        {simulation.length > 0 && (
          <div className="mt-6 border-t border-gray-100 pt-4">
            ...
            >
              ...
              {loading ? 'Guardando...' : 'Crear préstamo y descargar contrato'}
            </button>
          </div>
        )}
      </form>
